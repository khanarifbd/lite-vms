...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const result = await authenticatedBackendFetch(
      `/vehicles/owner-registration/${encodeURIComponent(vehicleId)}/drivers/assign`,
      {
        method: "POST",
...
export async function DELETE(request: Request) {
  ...
  try {
    const result = await authenticatedBackendFetch(
      `/assignments/${encodeURIComponent(payload.assignment_id)}/end`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          notes: payload.notes?.trim() || "Driver unassigned by vehicle owner",
        }),
      }
    )
    return NextResponse.json(result)
